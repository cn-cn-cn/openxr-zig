...
const Renderer = struct {
    ...
    fn renderSpecial(self: *Self, name: []const u8) !bool {
        ...
        } else if (std.mem.eql(u8, basename, "XrVersion")) {} else {
            return false;
        }
    ...
    fn renderEnumeration(self: *Self, name: []const u8, enumeration: reg.Enum) !void {
        if (enumeration.is_bitmask) {
            // will be rendered by renderBitmask
            return;
        }
    ...
    fn renderBitmaskBits(self: *Self, name: []const u8, bitmask: reg.Bitmask, bits: reg.Enum) !void {
        try self.writer.writeAll("pub const ");
        try self.renderName(name);
        const bitwidth = bitmask.bitwidth;
        ...
        if (bits.fields.len == 0) {
            ...
            for (flags_by_bitpos[0..bitwidth], 0..) |maybe_flag, bitpos| {
                if (maybe_flag) |flag| {
    ...
    fn renderBitmask(self: *Self, name: []const u8, bitmask: reg.Bitmask) !void {
        if (bitmask.bits_enum) |bits| {
            try self.renderBitmaskBits(bits, bitmask, self.decls_by_name.get(bits).?.enumeration);
        } else {
